...
UI/UX설계부터 프론트엔드와 백엔드 그리고 AX까지, 전 과정을 기초부터 다진 **AI 에이전트 엔지니어 (AI Agent Engineer)** 지망 개발자입니다.
...
|---|---|
| 거주지 | 경기도 성남시 |
| 이메일 | agii0114@naver.com |
| 자격증 | SQLD, 정보처리기사(필기), 컴퓨터활용능력 2급, GTQ 1급 등 |

[Tech Stack](#tech-stack) · [Projects](#projects)
...
이력서에 기재된 순서대로 정리된 5개의 프로젝트입니다. 각 프로젝트는 주제, 기획 의도, 아키텍처, 주요 작업, 기술 스택, 트러블슈팅을 다룹니다.
...
상단에는 룰 기반의 모니터링 및 재학습 에이전트와 ML 콘솔이 위치합니다. 주 사용자는 일반 소비자가 아닌 사내 유통 담당자입니다.

#### 기획 의도 (Planning Intent)

회사는 경매가가 저렴할 때 매수하여 저온 창고에 보관한 뒤, 가격이 상승하면 소비자가로 매도하는 *저장 유통(Storage Trading)*을 수행합니다. 이 스프레드 매매를 지원하기 위해서는 두 가지 다른 관점의 예측이 필요하지만, 농산물 가격은 변동성이 매우 큽니다 (배추 일일 변동성 13.97%, 태풍 힌남노 시기 한 달간 경매가 +68% 급등).
...
- **개발 중 ESG 단독에서 물류 ERP로 범위 피벗**<br>
...
- **Neo4j 그래프 데이터베이스의 공정별 집계 부적합**<br>
...
- **LLM 응답의 JSON 포맷 이탈로 인한 리포트 생성 실패**<br>
...
- **다른 권한으로 재로그인 시 이전 알림 구독이 유지되는 문제**<br>
...
저작권 문제를 고려하여 사설 엔드포인트 활용 및 커버 이미지 재호스팅은 명시적으로 제외했습니다.
...
- **테스트 서버의 운영 데이터베이스 잠금 현상 (~2분 장애)**<br>
...
- **재배포 시 라이브 터널 단락 현상 (HTTP 530)**<br>
...
### 5. High Go! — Apache Spark 기반 서울시 지하철 승하차 데이터 분석
